		...
		if (xhttp.status == 200) {
			...
			if (data != '') {
				...
				for (iter = 0; iter < data.length; iter++) {
					...
					var cod_pisoA = data[iter].cod_pisoA;
					...
					if (cod_pisoA == 0){
						...
						+ '<button type="submit" class="btn btn-primary btn-lg">Suscribirme</button> </form>'
					}
					html_user = html_user + '</div></div></div></div></div></div>';
